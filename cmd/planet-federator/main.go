...
func main() {
	...
	// Main
	flag.IntVar(&config.JobTimeoutSecond, "job-timeout-second", 30, "Timeout per federator job in second")
	...
	// Influxdb
	flag.StringVar(&config.InfluxdbAddress, "influxdb-address", "http://127.0.0.1:8086", "Target Influxdb HTTP Address")
	...
	// Prometheus
	flag.StringVar(&config.PrometheusAddr, "prometheus-addr", "http://127.0.0.1:9090/", "Prometheus address")
	...
	influxdbClient := influxdb2.NewClient(config.InfluxdbAddress, config.InfluxdbToken)
	influxdbHealth, err := influxdbClient.Health(ctx)
	if err != nil {
		log.Fatalf("Target Influxdb health-check error: %v", config.InfluxdbAddress, err)
	}
	if influxdbHealth.Status != influxdb2domain.HealthCheckStatusPass {
		log.Fatalf("Target Influxdb (%v) is unhealthy: %v", config.InfluxdbAddress, err)
	}
